//
//  GetDetailRecipeUseCaseTest.swift
...
class GetDetailRecipeUseCaseTest: XCTestCase {
    private var sut: GetDetailRecipeUseCase!
    @LazyInjected var recipesRepositories: RecipesRepositoriesType
    private var cancellable: AnyCancellable?
    private let idRecipe = "324334"
    ...
    func testGetDetailRecipeUseCase_ResponseSuccess() {
        //Given
        let expectationFailure = expectation(description: "failure")
        let expectation = self.expectation(description: "Consume Web Service Response Expectation")
        expectationFailure.isInverted = true
        
        RecipesRepositoriesStub.responseDetailRecipe = TestDomainConstants.recipeDetail
        
        //When
        cancellable = sut.execute(params: idRecipe)
            .sink { completion in
                ...
                // Then
                XCTAssertFalse(response.extendedIngredients.isEmpty)
                expectation.fulfill()
            }
    ...
    func testGetDetailRecipeUseCase_ResponseError() {
        //Given
        let expectationFailure = expectation(description: "failure")
        let expectation = self.expectation(description: "Consume Web Service Response Expectation")
